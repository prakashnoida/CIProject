...
class Home extends CI_Controller
{
    ...
    function profile($para1 = "", $para2 = "", $para3 = "")
    {
        ...
        }elseif ($para1 == "bids_awarded") {
            $this->load->view('front/user/bids_awarded');
    ...
    function wish_listed($para2 = '')
    {
        ...
        foreach($ids as $data)
        {
              ...
              $stock = $this->db->get_where('stock', array('product' => $prd_id));
              $stock_qty = $stock->row()->quantity;
              if($stock_qty > 10)
              {
                    $to = $user_email;
                    
                    $subject = "Wishlist Product Quantity Stock Is Less... ";
                    
                    $message = "Wishlist Product Quantity Stock Is Less... ";
                    ...
                    $config = array(
                        ...
                        'smtp_port' => 465,
                        'smtp_user' => 'chetanjadavtest@gmail.com',
                        'smtp_pass' => '5o73@529hW[~70751!GZ17b8u431Dv',
                        'mailtype'  => 'html',
                        'charset'   => 'utf-8'
                    );
                    ...
                    $this->email->to($to);
                    $this->email->from('testanviya@gmail.com','Ecommerce Bidding');
    ...
    function bids_awarded($para2 = '')
    {
        $this->load->library('Ajax_pagination');

        $id = $this->session->userdata('user_id');
        $ids = json_decode($this->db->get_where('user', array('user_id' => $id,'user_id' => 1))->row()->wishlist, true);
        $this->db->where_in('product_id', $ids);

        $config['total_rows'] = $this->db->count_all_results('product');;
        $config['base_url'] = base_url() . 'home/bids_awarded/';
        $config['per_page'] = 5;
        $config['uri_segment'] = 5;
        $config['cur_page_giv'] = $para2;
    ...
    function add_bid($para1 = '')
    {
       $id = $para1; 
       $bidtype = $this->input->post('bid_type');
       $new_bid = $this->input->post('target_amt');
       
       $product_data = $this->db->get_where('product', array('product_id' => $id));
       $base_amount =  $product_data->row()->auction_base_price;
       if($base_amount<$new_bid)
       
       {
       
           $product_data = $this->db->get_where('tbl_auction', array('heighest_auction' => 1 ,'product_id' => $id));
           $last_bid_amount =  $product_data->row()->bid_amount;
    
           if($last_bid_amount<$new_bid)
           {
            if($bidtype=='proxy')
            {
                /* Insert Bid Amount */
                ...
                $data['auction_type'] = $this->input->post('bid_type');
                $data['bid_amount'] = $this->input->post('target_amt');
                $data['bid_interval'] = $this->input->post('target_interval');
                $data['bid_limit'] = $this->input->post('target_max_amt');
                $data['heighest_auction'] = 0;
    
                $this->db->insert('tbl_auction', $data);
                $last_id=$this->db->insert_id();
    
                /* Last Heighst Bid Amount To Bid Amount Update */
    
    
                        $product_data = $this->db->get_where('tbl_auction', array('heighest_auction' => 1,'product_id' => $id));
                        $product_data_count = $this->db->get_where('tbl_auction', array('product_id' => $id));
    
                        $last_auction_id = $product_data->row()->auction_id;
                        $last_bid_amount =  $product_data->row()->bid_amount;
                        $last_bid_limit =  $product_data->row()->bid_limit;
                        $last_bid_interval =  $product_data->row()->bid_interval;
                        $last_auction_type =  $product_data->row()->auction_type;
                        $new_bid_amount=$this->input->post('target_amt');
                        $new_auction_type=$this->input->post('bid_type');
                        $new_bid_interval=$this->input->post('target_interval');
                        $new_bid_limit=$this->input->post('target_max_amt');
    
                        if($new_auction_type != 'fixed')
                        {
                            if ($product_data_count->num_rows() != 1)
                            {
                               
                                if($last_bid_amount<$new_bid_amount)
                                {
    
                                    
                                    if($last_bid_limit==$new_bid_limit)
                                    {
                                        
                                        $new_bid_amount_old = $last_bid_limit;
                                        $new_bid_amount_new = $new_bid_limit;
    
                                        $this->db->where('auction_id', $last_auction_id);
                                        $this->db->update('tbl_auction', array(
                                            'bid_amount' => $new_bid_amount_old
                                        ));
    
                                        $auction_new_id = $last_id;
                                        $this->db->where('auction_id', $auction_new_id);
                                        $this->db->update('tbl_auction', array(
                                            'bid_amount' => $new_bid_amount_new
                                        )); 
                                    } 
                                    else if($last_bid_limit<$new_bid_limit)
                                    {
                                        
                                        $new_bid_amount_old = $last_bid_limit;
                                        $new_bid_amount_new = $last_bid_limit + $new_bid_interval;
    
                                        if($new_bid_limit<$new_bid_amount_new)
                                        {
                                            $new_bid_amount_old = $last_bid_limit;
                                            $new_bid_amount_new = $new_bid_limit;
                                        }
                                        ...
                                        {
                                            $new_bid_amount_old = $last_bid_limit;
                                            $new_bid_amount_new = $new_bid_amount_new;
                                        }
                                    ...
                                    {
                                        $new_bid_amount_old = $new_bid_limit + $last_bid_interval; 
                                        $new_bid_amount_new = $new_bid_limit;
                ...
                /* Last Heighst Bid Amount To Heightst Status Update */
    
                $product_data = $this->db->order_by('bid_amount', 'desc')->get_where('tbl_auction', array('product_id ' => $id , 'heighest_auction' => 1 ));
                ...
                $product_data = $this->db->order_by('bid_amount', 'desc')->get_where('tbl_auction', array('product_id ' => $id));
                $auction_id = $product_data->row()->auction_id;
            ...
            else
            {
                ...
                $this->db->insert('tbl_auction', $data);
                $auction_new_id = $this->db->insert_id();
                ...
                if($new_bid_limit==0)
                {
                    ...
                    if($new_bid_amount<$last_bid_limit)
                    {
                        $new_bid_amount_old = $new_bid_amount + $last_bid_interval; 
                        $new_bid_amount_new = $new_bid_amount;
           ...
           {
                $this->session->set_flashdata('flashSuccess', 'Bid Amount Greater Of The Last Bid Amount');
           }
           
       }
       else
           {
                $this->session->set_flashdata('flashSuccess', 'Bid Amount Greater Of The Base Bid Amount');
           }
        

       redirect('home/product_view/'.$id);
       
       
    }

     function update_bid($para1 = '')
    {
       $id = $para1; 
       ...
       if($last_bid_amount<$new_bid)
       {
        if($bidtype=='proxy')
        {
            ...
            $this->db->where('auction_id', $auction_id);
            $this->db->update('tbl_auction', array(
                'auction_type' => $this->input->post('bid_type'),
                'bid_amount' => $this->input->post('target_amt'),
                'bid_interval' => $this->input->post('target_interval'),
                'bid_limit' => $this->input->post('target_max_amt'),
